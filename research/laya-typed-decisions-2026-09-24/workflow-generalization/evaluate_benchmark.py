...
def score_predictions(rows: list[dict], latencies: list[float]) -> dict:
    ...
    by_type_dist_metrics = defaultdict(lambda: defaultdict(list))
    for row in rows:
        qtype = row["kind"]
        ...
        for name, value in (("target_cross_entropy", ce), ("kl_from_gold", kl),
            ...
            by_type_dist_metrics[qtype][name].append(value)
    ...
    scores_out = {
        ...
        "by_type_metrics": {k: {name: statistics.fmean(values) for name, values in sorted(v.items())}
                            for k, v in sorted(by_type_dist_metrics.items())},
...
def main() -> None:
    ...
    ap.add_argument("--uncalibrated", action="store_true", help="disable checkpoint temperature scaling")
    args = ap.parse_args()
    ...
    if rank == 0:
        ...
        summary = {"label": args.label, "mode": args.mode,
                   "calibration": "disabled" if args.uncalibrated else "checkpoint default",
                   "applied_temperature_by_type": agent.temperature,
                   "applied_temperature_by_options": agent.temperature_by_options,
